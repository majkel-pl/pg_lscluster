...
sub check_pidfile_running {
    ...
    if (defined $pid) {
                if (open CL, "/proc/$pid/cmdline") {
                        my $cmdline = <CL>;
                        close CL;
                        if ($cmdline and $cmdline =~ /\bpgsql\b/) {
                                return 1;
                        } else {
                                return 0;
                        }
